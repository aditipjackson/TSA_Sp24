...
raw_inflow_data <- read.table("./Data/inflowtimeseries.txt",header=FALSE,skip=0)
...
```
> moving average looks six months forward and six months backwards
> see Amazon inflow data PDF from Module X for code on how to calculate a moving average
...
> ts clean can be used for outliers and missing data
> forecast: na.inter() or zoo: na.approx() <-- helps you take care of N/As only
...
> decomposition function gives you a centered moving average 
> in 4th part - random is what you have left after removing trend and seasonality 
> when decompose series, you're going to end up with some N/As that were not there before
>> b/c of moving average - start and finish series 6 months before and after so lose some variables, need to start your series 6 months in (see gaps in front and back of dataset); don't have 6 months of data to calculate off of
...
#Writing a "while" loop to remove all outliers
...
> advantage of using this loop instead of ts-clean is that you can change the p-value and also specify the number of iterations (I only want to run this 4x)
