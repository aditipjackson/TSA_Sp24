...
```{r}
ts_inflow_data <- ts(inflow_data[,"HP3"],start=c(1931,1),frequency=12)
# first argument is what you're trying to convert to time series; you can use the column # or column name. So use 2 instead of "HP1"
# first space before comma reffers to # of rows; if you leave it blank it'll give you all the rows
# frequency = number of times observation repeats
...
HP1_acf <- Acf(ts_inflow_data) # output is a plot
# plot shows a periodic pattern, gives you info that a seasonal component is present in the data set
# note that "acf()" with small a starts at 0, "Acf()" with a capital A gives months
...
HP1_pacf <- Pacf(ts_inflow_data,lag.max=40) # look at PACF to point out which elements to add to model b/c have strong correlation with value you're trying to estimate; blue lines represent "significant" correlation interval, should add things outside of the blue lines
