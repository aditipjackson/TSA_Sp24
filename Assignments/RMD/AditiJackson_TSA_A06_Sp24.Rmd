...
> Answer: The ACF plot of an AR model decays exponentially with time. In this case, the order of the series is 2, which is given by the PACF. The PACF for AR(2) will indicate that autocorrelation is only significant up to the second lag. Graphically we see this as the first two bars being above the sigificance cut off and all other lags being within the cutoff.
...
ARMA(0,1): I'm not as certain I'd be able to identify this as MA. By looking at the ACF I can tell that the order is 1 given the sigicant lag. However, it's hard to see an exponential decay in the PACF, which would make me less certain of verifying this as MA.
...
> Answer:
ARMA(1,0): Lag one of the PACF, which gives the correlation coefficient values for AR models, has a magnitude of 0.6 which matches the given Phi. This seems to an ideal situation where the population parameter (Phi) is equal to the sample estimate given by the magnitude of lag 1.
ARMA(0,1): Lag one of the ACF, which gives the value of the Theta has a magnitude of ~0.4. It does not match the population parameter (Theta), which equals 0.9. This is likely becasue we have only run the model for n=100. Perhaps running it with more observations will give us a sample estimate closer to theta.
ARMA(1,1): Lag 1 of the PACF gives us a value of ~0.77, which is almost halfway between phi=0.6 and theta=0.9. [ADD]

(f) Increase number of observations to $n=1000$ and repeat parts (b)-(e).

```{r repeating b through e}
# Generating 100 observations for each ARMA model
## AMRA(1,0)
...
> Answer:
ARMA(1,0): The given still phi matches the PACF value in this case.
ARMA(0,1): The sample estiamte is still below the below the population parameter, but increased above 0.4. This is likely because we ran the model with more observations (law of large numbers).
ARMA(1,1):[ADD]
