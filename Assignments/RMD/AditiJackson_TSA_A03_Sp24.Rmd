...
> Renewable energy production has a notable increasing trend based on the time series plot. 
...
ggplot(df_renew_detrend,aes(x=date))+
  ...
  labs(x="Date",y="Observations",title="Renewable Energy Production - Detrended")
...
> Comparing the detrended plots to the originals in Q1, renewable energy production shows a XXX. Hydroelectric power consumption XXX. 
...
> For renewables production, the ACF and the PACF look very similar to Q1. For hydroelectric consumption, the ACF and the PACF also look similar.
...
```
> Only the intercept Beta0 appears to be significant. The R-squared value tells us that only 9.26% of variability in the data is explained by seasonality. This tells us that total renewable energy production does not appear to have a seasonal trend.
...
```
> Beta0 (Dec), Beta5 (May), Beta6 (June),Beta9 (Sept), Beta10 (Oct), and beta11 (Nov) appear to be highly significant with p values less than 0.001. The R-squared value tells us that 46.97% of variability in the hydroelectric power consumption can be explained by seasonaility.

### Q8
Use the regression coefficients from Q7 to deseason the series. Plot the deseason series and compare with the plots from part Q1. Did anything change?
```{r deseasoning series - renewable energy prod}
...
REP_deseason_plot <- ggplot(renewable_energy_sub,aes(x=Date,y=Total.Renewable.Energy.Production))+
...
plot_grid(
  autoplot(Renewables_tsplot),
  autoplot(REP_deseason_plot),
  nrow=1,ncol=2
)

```
> Plotting the original data set against the deseasoned data set, the graphs appear the same with just one shifted down.


```{r deseasoning series - hydro power consump}
# Hydroelectric Power Consumption
## storing regression coefficients in order to de-season data
...
## plotting deseasoned data
HPC_deseason_plot <- ggplot(renewable_energy_sub,aes(x=Date,y=Hydroelectric.Power.Consumption))+
  geom_line(color="blue")+
...
plot_grid(
  autoplot(Renewables_acf,main=NULL),
  autoplot(Renewables_pacf,main=NULL),
  autoplot(REP_deseasoned_acf,main=NULL),
  autoplot(REP_deseasoned_pacf,main=NULL),
  nrow=2, ncol=2
)
...
plot_grid(
  autoplot(HydroConsump_acf,main=NULL),
  autoplot(HydroConsump_pacf,main=NULL),
  autoplot(HPC_deseasoned_acf,main=NULL),
  autoplot(HPC_deseasoned_pacf,main=NULL),
  nrow=2, ncol=2)

```
> My ACF and PACF plots do not appear to have changed at all for either time series.
